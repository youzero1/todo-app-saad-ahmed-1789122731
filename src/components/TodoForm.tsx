...
export function TodoForm({ onAdd }: TodoFormProps) {
  ...
  return (
    <form onSubmit={submit} className="flex gap-3 rounded-2xl border border-slate-200 bg-white p-2 shadow-[0_10px_30px_rgba(45,55,90,0.06)]">
      ...
      <button type="submit" className="rounded-xl bg-[var(--accent)] px-5 py-3 text-sm font-bold text-white transition hover:bg-[var(--accent-hover)] disabled:cursor-not-allowed disabled:opacity-50" disabled={!text.trim()}>Add task</button>
    </form>
  );
}
