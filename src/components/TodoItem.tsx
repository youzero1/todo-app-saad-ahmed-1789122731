...
export function TodoItem({ todo, onToggle, onEdit, onDelete }: TodoItemProps) {
  ...
  return (
    <li className="group flex items-center gap-3 border-b border-slate-100 py-4 last:border-0">
      <button type="button" aria-label={todo.completed ? `Mark ${todo.text} incomplete` : `Complete ${todo.text}`} onClick={onToggle} className={`grid h-6 w-6 shrink-0 place-items-center rounded-full border-2 transition ${todo.completed ? 'border-red-600 bg-red-600 text-white' : 'border-slate-300 hover:border-red-600'}`}>
